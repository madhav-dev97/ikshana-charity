...
import defaultImageUrl from "@/assets/images/cause-edu.png"; // Fallback image
...
export default function Causes() {
  ...
  return (
    <div className="container px-4 py-12 md:py-20 animate-in fade-in duration-500">
      <div className="max-w-3xl mb-12">
        ...
        <p className="text-xl text-muted-foreground leading-relaxed">
          ...
          Explore the history of our community's impact.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {sortedCauses.map((cause) => {
            ...
            return (
              <div 
                key={cause.id} 
                className={`group flex flex-col rounded-2xl border bg-card shadow-sm hover:shadow-md transition-all overflow-hidden ${
              ...
              >
                <div className="relative h-56 overflow-hidden bg-muted">
                  ...
                  <div className="absolute top-4 right-4 flex gap-2">
                    ...
                    {isFunded && !isCurrent && (
                      <Badge variant="secondary" className="bg-secondary text-secondary-foreground font-semibold px-3 shadow-md">
                        Fully Funded
                      </Badge>
                    )}
                  </div>
                  ...
                  <div className="absolute bottom-4 left-4 text-white">
                    <Badge variant="outline" className="text-white border-white/30 bg-black/30 backdrop-blur-md mb-2">
                      {cause.category}
                    </Badge>
                  </div>
                </div>
                  ...
                  <div className="space-y-4 mt-auto">
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span className="font-semibold">${cause.raisedAmount.toLocaleString()} raised</span>
                        <span className="text-muted-foreground flex items-center gap-1">
                          <Target className="w-3 h-3" /> Goal: ${cause.goalAmount.toLocaleString()}
                        </span>
                      </div>
                      <Progress 
                        value={progress} 
                        className="h-2 rounded-full bg-secondary/20"
                    ...
                    {isCurrent ? (
                      <Link href="/donate" className="block w-full">
                        <Button className="w-full rounded-xl shadow-sm">
                          Donate Now
                        </Button>
                      </Link>
                    ) : (
                      <div className="flex flex-col gap-2 pt-2 border-t">
                        <div className="text-xs text-muted-foreground">
                          <span className="font-semibold text-foreground">Impact:</span> {cause.impact || "Details coming soon"}
                        </div>
                      </div>
                    )}
                  </div>
